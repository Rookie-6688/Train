...
<body style="background:url(img/fjbj.jpg);background-repeat:no-repeat ;background-size:100% 100%; background-attachment: fixed;">
<%	
/* 用户名 */
  ...
  <input type="file" name="fileToUpload" id="fileToUpload" Onchange="fileSelected()" style="display:none;">
    ...
    <section id="main-content">
      <section class="wrapper">
      	<table border="1px" width="700px" height="500px" align="center" class="font">
      	<tr align="center">
      	...
      	<tr align="center">
      		...
      		<%for(int m=0;m<map.get(train.getname(list.get(i))).size();m++){ %>
	      		...
	      		<%if(map.get(train.getname(list.get(i))).get(m).equals(first)||map.get(train.getname(list.get(i))).get(m).equals(end)) {%>
	      			<td style="background-color:white;opacity:0.6;color:black"><div style="color:red"><%=map.get(train.getname(list.get(i))).get(m) %></div><div style="color:red">出发时间:<%=train_l.get(train.getname(list.get(i))).get(m)%></div></td>
	      		<%}else{ %>	
	      			<td style="background-color:white;opacity:0.6;color:black"><div><%=map.get(train.getname(list.get(i))).get(m) %></div><div>出发时间:<%=train_l.get(train.getname(list.get(i))).get(m) %></div></td>
	      		<%} %>
	      		
      		<%} %>
	      		<td width="100px" style="background-color:white;opacity:0.6;color:black">
	      		票价:<%=numberstation.get(train.getname(list.get(i)))*100%><a href="planedetails.jsp?i=<%=train.getname(list.get(i))%>&&type=1">购票</a>
	      		</td>
      	</tr>
      	<%} %>
      	</table>
